...
for csv_file in csv_files:
    ...
    if 'UpdateTime' in csv_df.columns.values:
        ...
        csv_df['Datetime'] = pd.to_datetime(csv_df['TradeDate'].map(str) + csv_df['UpdateTime'].map(str),
                                            format='%Y%m%d%H%M%S%f')
        csv_df['Qty'] = csv_df['Volume'].cumsum()
    else:
        # snapshot
        csv_df['Datetime'] = pd.to_datetime(csv_df['TradeDate'].map(str) + csv_df['DataTime'].map(str),
                                            format='%Y%m%d%H%M%S%f')
...
for date, data in csv_dfs.items():
    ...
    for j in snapshot.index:
        index = np.where(tick.Volume == snapshot.Volume[j])[0]
    ...
    for i in range(len(snapshot.index) - 1, -1, -1):
        if snapshot[snapshot.index == snapshot.index[i]].Datetime.iloc[0] == \
                snapshot[snapshot.index == snapshot.index[i - 1]].Datetime.iloc[0]:
