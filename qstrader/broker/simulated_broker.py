...
class SimulatedBroker(Broker):
    ...
    def _set_initial_market_making_statistic(self):
        ...
        # count每秒查询 挂单市值 
        market_making_statistic = {i: {'count': {'bid': deque(), 'ask': deque()}, 'result': None} for i in
                                   self.data_handler.data_source.data_info['Days']}
    ...
    def get_market_making_statistic(self):
        ...
        market_making_statistic = {i: {'count': {'bid': deque(), 'bid_count': 0, 'ask': deque(), 'ask_count': 0}} for i
                                   in self.data_handler.data_source.data_info['Days']}
        ...
        if kwargs['order_type'] == 'delay':
            if self.lims_flag == 2:
                if 'sell_price' in kwargs.keys():
                    ...
                    kwargs['sell_volume'] = min(kwargs['sell_volume'],
                                                self.portfolio['total_volume'] - self.initial_portfolio['total_volume'])
                ...
                if 'buy_price' in kwargs.keys():
                    ...
                    # 再更新买单
                    kwargs['buy_volume'] = min(kwargs['buy_volume'],
                                               self.initial_portfolio['total_volume'] - self.portfolio['total_volume'])
    ...
    def _check_order_delay(self, order_type, order_delay_list):
        ...
        if order_type == 'cancel':
            order_delay_list['bid'] = deque(
                sorted(order_delay_list['bid'], key=lambda kv: (kv[1], kv[0]), reverse=True))
        ...
        if order_type == 'cancel':
            order_delay_list['ask'] = deque(
                sorted(order_delay_list['ask'], key=lambda kv: (kv[1], kv[0]), reverse=False))
    ...
    def _exec_update_order_delay(self):
        ...
        if (n := len(update_order_delay_list['bid'])) > 0:
            for i in range(n):
                if update_order_delay_list['bid'][i][3] > self.cash:
                    continue
                if (order_time := update_order_delay_list['bid'][i][0] + datetime.timedelta(
                        milliseconds=self.time_delta)) <= self.current_time:
                        ...
                        for j_tick in latest_data:  # latest_data 已逆序
                            if j_tick[0] <= update_order_delay_list['bid'][i][0]:
                                ...
                                if j_tick[1] <= update_order_delay_list['bid'][i][1]:
                                    sim_volume = min(update_order_delay_list['bid'][i][2],
                                                     self._simulated_volume(j_tick[2]))
        ...
        if (n := len(update_order_delay_list['ask'])) > 0:
            for i in range(n):
                if (order_time := update_order_delay_list['ask'][i][0] + datetime.timedelta(
                        milliseconds=self.time_delta)) <= self.current_time:
                        ...
                        for j_tick in latest_data:  # latest_data已逆序
                            if j_tick[0] <= update_order_delay_list['ask'][i][0]:
                                ...
                                if j_tick[1] >= update_order_delay_list['ask'][i][1]:
                                    ...
                                    sim_volume = min(update_order_delay_list['ask'][i][2],
                                                     self._simulated_volume(j_tick[2]))
                                    ...
                                    # 不论是否完全成交 该单均撤销
                                    update_order_delay_list['bid'][i] = [0, 0, 0, 0]
    ...
    def _exec_cancel_order_delay(self):
        ...
        for j in range(len(latest_data)):
            if (m := len(cancel_order_delay_list['bid'])) > 0:
                for i in range(m):
                    # 延迟内
                    if cancel_order_delay_list['bid'][i][0] + datetime.timedelta(
                            milliseconds=self.time_delta) >= self.current_time:
                        # 买量不为0，买价 >= 最新价   [1] price  [2] volume
                        if cancel_order_delay_list['bid'][i][2] != 0 and cancel_order_delay_list['bid'][i][1] >= \
                                latest_data[j][1]:
                            if cancel_order_delay_list['bid'][i][1] == latest_data[j][1]:
                                sim_volume = min(cancel_order_delay_list['bid'][i][2],
                                                 self._simulated_volume(latest_data[j][2]))
                            else:
                        ...
                        if cancel_order_delay_list['bid'][i][2] != 0:
                            ...
                            self._add_data_to_order_list(
                                cancel_order_delay_list['bid'][i][0] + datetime.timedelta(milliseconds=self.time_delta))

                cancel_order_delay_list = self._check_order_delay('cancel', cancel_order_delay_list)
            elif (m := len(cancel_order_delay_list['ask'])) > 0:
                for i in range(m):
                    # 延迟内
                    if cancel_order_delay_list['ask'][i][0] + datetime.timedelta(
                            milliseconds=self.time_delta) >= self.current_time:
                        # 卖量不为0, 卖价 <= 最新价    [4] price  [5] volume
                        if cancel_order_delay_list['ask'][i][2] != 0 and cancel_order_delay_list['ask'][i][1] <= \
                                latest_data[j][1]:
                            if cancel_order_delay_list['ask'][i][1] == latest_data[j][1]:
                                sim_volume = min(cancel_order_delay_list['ask'][i][2],
                                                 self._simulated_volume(latest_data[j][2]))
                            else:
                        ...
                        if cancel_order_delay_list['ask'][i][2] != 0:
                            ...
                            self._add_data_to_order_list(
                                cancel_order_delay_list['ask'][i][0] + datetime.timedelta(milliseconds=self.time_delta))
    ...
    def _update_current_time(self, **kwargs):
        ...
        self.current_time = pd.to_datetime(
            self.data_handler.data_source.data[self.current_day]['snapshot']['data'][self.current_k][0])
    ...
    def _check_order_time(self, **kwargs):
        ...
        if (n := len(order['bid'])) > 0:
            for i in range(n - 1, -1, -1):
                if (order_time := order['bid'][i][0] + datetime.timedelta(
                        seconds=keep_order_time + order_delay_time)) < self.current_time:
                    # 直接撤单
                ...
                cancel_time := order['bid'][i][0] + datetime.timedelta(seconds=keep_order_time)) <= self.current_time:
                    # 撤单延迟
        ...
        if (n := len(order['ask'])) > 0:
            for i in range(n - 1, -1, -1):
                if (order_time := order['ask'][i][0] + datetime.timedelta(
                        seconds=keep_order_time + order_delay_time)) < self.current_time:
                    # 直接撤单
                ...
                cancel_time := order['ask'][i][0] + datetime.timedelta(seconds=keep_order_time)) <= self.current_time:
                    # 撤单延迟
                    self._cancel_order_delay(-1,
                                             cancel_time,
                                             order['ask'][i][1:])
        ...
        for stock in self.stocks:
            if order[stock]['buy_price'] * order[stock]['buy_volume'] > order[stock]['frozen_money']:
                raise ValueError('(%s) order error: buy_price: %s; buy_volume: %s; frozen_money: %s' % (stock,
                                                                                                        ...
                                                                                                        order[stock][
                                                                                                            'frozen_money']))
            if portfolio[stock]['total_volume'] != portfolio[stock]['available_volume'] + portfolio[stock][
                'frozen_volume'] or portfolio[stock]['frozen_volume'] < 0:
                raise ValueError(
                    '(%s) portfolio error: total_volume: %s; available_volume: %s; frozen_volume: %s' % (stock,
                                                                                                         portfolio[
                                                                                                             stock][
                                                                                                             'total_volume'],
                                                                                                         portfolio[
